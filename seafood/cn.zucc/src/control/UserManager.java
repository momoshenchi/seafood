...
public class UserManager
{
    ...
    public List<BeanOrder> loadAllUserOrders()
    {
        Connection con = null;
    ...
    // discount ,discountid
    public  void userShop (int commodityid , int number ,double price ,double discount,int discountid)
    {
        int userid = BeanUser.currentLoginUser.getUserid();
        Connection con = null;
        try
        {
            ...
            rs.close();
            pst.close();
            sql="insert  into order_detail(orderid,commodityid,number,price,discount,discountid,userid) values(" +
                "?,?,?,?,?,?,?)";
            pst = con.prepareStatement(sql);
